...
### 17.5 Сравнение: Zero-shot vs Обученные на final_dataset

| Модель | Zero-shot AUC | Обученные AUC | Разница |
|--------|---------------|---------------|---------|
| **Embedding KNN** | 0.956 | 0.941 | -0.015 |
| **Ranking** | 0.940 | 0.958 | +0.018 |
| **Ensemble** | 0.957 | 0.942 | -0.015 |

**Выводы:**
1. **Ranking модель улучшилась** после обучения на final_dataset (+0.018 AUC)
2. **Embedding KNN немного ухудшился** (-0.015 AUC), но всё ещё показывает отличный результат
3. **Zero-shot модели** (обученные на английских датасетах) работают почти так же хорошо, как обученные на русских
4. Это подтверждает, что **паттерны AI-текстов универсальны** для разных языков
...
### 17.8 Рекомендации для final_dataset

1. **Использовать Ranking модель как основную** - лучший результат (AUC = 0.958)
2. **Добавить r_detect_final_dataset** в ансамбль после её обучения
