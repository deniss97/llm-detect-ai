...
**Метрики на тесте (threshold=0.0001):**
| Метрика | Значение |
|---------|----------|
| ROC-AUC | **0.6586** |
| Accuracy | 0.6755 |
...
|--------|---------|----------|
| **human** | 125 | **39.2%** (много false positive) |
| **google/gemini-3.1-flash-lite** | 41 | **100.0%** |
| **qwen/qwen-2.5-72b-instruct** | 43 | **93.0%** |
| **openai/gpt-4o-mini** | 38 | **97.4%** |
...
**Наблюдения:**
1. **Модель не переобучилась** (в отличие от предыдущей Mistral-7B с AUC=1.0)
2. **ROC-AUC 0.66** - реалистичная оценка качества без data leak
3. **Отличное обнаружение AI текстов:** Recall 96.7%, особенно для Gemini (100%) и GPT-4o-mini (97.4%)
4. **Много ложных срабатываний** на human текстах (Precision 60.8%)
5. **Оптимальный threshold = 0.0001** (очень низкий) - модель выдает низкие вероятности для AI текстов
...
| **T-lite-7B (новая)** | **0.6586** (тест) | **0.6076** | **0.9672** | ❌ Нет (сплит по source_id) |

**Вывод:** T-lite модель показывает более реалистичные метрики благодаря исправленному сплиту данных (без data leak).
