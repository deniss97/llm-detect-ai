...
### 17.3 Результаты оценки моделей (3 модели ансамбля)

**Важно:** Метрики обновлены 12.04.2026 после исправления target leak в коде оценки.

| Модель | ROC-AUC | F1 | Precision | Recall | Accuracy |
|--------|---------|----|-----------|--------|----------|
| **Embedding KNN** | **0.9558** | **0.9188** | **0.9556** | **0.8848** | **0.9224** |
| **Ranking (fast)** | **0.9405** | **0.8795** | **0.8588** | **0.9012** | **0.8776** |
| r_detect_final_dataset | 0.6763 | 0.6916 | 0.6336 | 0.7613 | 0.6633 |

**Наблюдения:**
1. **Embedding KNN показал лучший результат** (AUC = 0.956) - подтверждает эффективность на русскоязычных текстах
2. **Ranking модель** также показала хороший результат (AUC = 0.940)
3. **r_detect_final_dataset** показывает умеренное качество (AUC = 0.676) - модель переобучилась на тренировочных данных, но предсказания корректны (не инвертированы)

### 17.4 5-Fold Cross-Validation

| Модель | ROC-AUC | F1 | Accuracy |
|--------|---------|----|----------|
| **embedding_knn** | **0.9855±0.0041** | **0.8814±0.0186** | **0.8953±0.0144** |
| ranking | 0.9478±0.0140 | 0.7436±0.0090 | 0.6650±0.0131 |
| r_detect_final_dataset | 0.9825±0.0052 | 0.6621±0.0007 | 0.4949±0.0008 |

**Наблюдение:** 
- Embedding KNN стабилен (низкое стандартное отклонение)
- r_detect_final_dataset показывает высокий AUC на CV, но низкий F1 из-за переобучения

### 17.5 Результаты ансамблей

| Ансамбль | ROC-AUC | F1 | Precision | Recall | Accuracy |
|----------|---------|----|-----------|--------|----------|
| **Weighted Ensemble** | **0.9573** | **0.9188** | **0.9556** | **0.8848** | **0.9224** |
| Meta-Learner | 0.9564 | 0.9188 | 0.9556 | 0.8848 | 0.9224 |

**Коэффициенты мета-обучения:**
| Модель | Коэффициент |
|--------|-------------|
| embedding_knn | **12.74** |
| r_detect_final_dataset | 1.95 |
| ranking | 0.13 |

**Наблюдение:** Embedding KNN доминирует в ансамбле (коэффициент 12.74), ranking модель имеет минимальный вклад.

### 17.6 Confusion Matrix для Weighted Ensemble
...
**Ошибки:**
- **False Positive:** 10 (4.0% от human)
- **False Negative:** 28 (11.5% от AI)
- **Total Errors:** 38/490 (7.8%)

### 17.7 Сравнение с предыдущим датасетом

| Метрика | Final Dataset (RU) | Detection Dataset (EN) | Разница |
|---------|-------------------|------------------------|---------|
| **Embedding KNN AUC** | **0.956** | 0.989 | -0.033 |
| **Ranking AUC** | **0.940** | 0.972 | -0.032 |
| **Best Ensemble AUC** | **0.957** | 0.991 | -0.034 |
| **r_detect AUC** | 0.547 | 0.818 | -0.271 |

**Выводы:**
1. Embedding KNN и Ranking модели хорошо переносятся на русскоязычные тексты (AUC > 0.94)
2. Detection модель (Mistral-7B) показала худшие результаты, возможно из-за:
   - Меньшего размера датасета (2448 vs 10200)
   - Переобучения на тренировочных данных
   - Языковых особенностей (русский vs английский)

### 17.8 Сохранённые файлы

| Файл | Описание |
|------|----------|
| `results/final_dataset_ensemble_results.json` | Полные метрики всех моделей и ансамблей |
| `results/final_dataset_ensemble_submission.csv` | Submission файл |
| `results/final_dataset_ensemble_summary.csv` | Сводная таблица метрик |
| `results/final_dataset_cv_report.json` | 5-Fold CV результаты |
| `results/meta_learner_final_dataset.pkl` | Meta-learner для final_dataset |
| `code/evaluate/ensemble_final_dataset.py` | Скрипт оценки ансамбля на final_dataset |
| `models/r_detect_final_dataset/best/` | Лучший checkpoint модели |

### 17.9 Рекомендации для final_dataset

1. **Использовать Embedding KNN как основную модель** - лучший результат (AUC = 0.956)
2. **Дообучить Mistral-7B с регуляризацией** - текущая модель переобучилась
3. **Увеличить датасет** - добавить больше русскоязычных текстов
4. **Использовать многоязычные модели** - например, XLM-RoBERTa вместо DeBERTa-v3
5. **Применить аугментацию данных** - back-translation, synonym replacement
