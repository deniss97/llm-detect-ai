...
def run_training(cfg):
    ...
    # Check for external dataset (for embed model)
    ...
    try:
        ...
        # Create valid split from train if no valid exists
        rng = random.Random(cfg.seed)
        train_df['fold'] = train_df['text'].apply(lambda x: 'train' if rng.random() < 0.99 else 'valid')
        valid_df = train_df[train_df['fold'] == 'valid'].copy()
    
    valid_df = valid_df[~valid_df['text'].isna()].copy()
    
    # Load external dataset if specified (for embed model)
    if external_data_dir is not None and os.path.exists(external_data_dir):
        ...
        accelerator.print(f"Combined train dataset shape: {train_df.shape}")
