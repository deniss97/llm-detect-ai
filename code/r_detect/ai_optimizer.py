# bitsandbytes import removed to avoid CUDA setup issues
# Using standard PyTorch optimizers instead
...
def get_optimizer(cfg, model, print_fn=None):
    ...
    # Fall back to AdamW if AdamW8bit requested
    if cfg.optimizer.name == "AdamW8bit":
        if print_fn:
            print_fn("WARNING: AdamW8bit requested but bitsandbytes not available. Using AdamW instead.")
