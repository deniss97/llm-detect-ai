#!/usr/bin/env python3
"""
Extended Ensemble Evaluation:
- Detection models (Mistral-7B LoRA)
- Embedding model (DeBERTa-v3-Base + KNN)
- Ranking model (DeBERTa-v3-Large)
...
def get_embedding_knn_predictions(train_df, test_df, k=5):
...
def main():
    print("="*70)
    print("EXTENDED ENSEMBLE EVALUATION")
    print("="*70)
    ...
    if emb_train is not None:
        ...
        # For val, use train as reference (simplified)
    ...
    meta_model = train_meta_learner(
        {k: v for k, v in all_preds['train'].items() if k in all_preds['val']},
        y_train
    )
    
    # Prepare features for val and test
    val_features = np.array([all_preds['val'][k] for k in meta_model.feature_names_in_]).T
    test_features = np.array([all_preds['test'][k] for k in meta_model.feature_names_in_]).T
    
    meta_val_probs = meta_model.predict_proba(val_features)[:, 1]
    meta_test_probs = meta_model.predict_proba(test_features)[:, 1]
    
    print("\n  Meta-learner coefficients:")
    for name, coef in zip(meta_model.feature_names_in_, meta_model.coef_[0]):
    ...
    submission_df = pd.DataFrame({
        ...
        'is_generated': meta_test_probs,
    })
    submission_df.to_csv(os.path.join(RESULTS_DIR, "ensemble_submission.csv"), index=False)
