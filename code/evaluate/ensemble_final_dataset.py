#!/usr/bin/env python3
"""
Ensemble Evaluation on Final Dataset:
- r_detect_final_dataset (Mistral-7B + LoRA) - trained on final_dataset
...
# Model configs - ONLY 3 models for final_dataset ensemble
DETECT_PREDICTIONS = {
    'r_detect_final_dataset': {
        'path': os.path.join(MODELS_DIR, 'r_detect_final_dataset/best'),
...
# Only these 3 models will be used in ensemble
ENSEMBLE_MODELS = ['r_detect_final_dataset', 'embedding_knn', 'ranking']


def load_data():
    """Load train, valid datasets from final_prepared."""
    print("Loading datasets...")
    train_df = pd.read_csv(os.path.join(DATA_DIR, "final_prepared/final_train.csv"))
    valid_df = pd.read_csv(os.path.join(DATA_DIR, "final_prepared/final_valid.csv"))
    
    # Convert label/generated to is_generated (0=human, 1=ai)
    if 'label' in train_df.columns:
        train_df['is_generated'] = (train_df['label'] == 'ai').astype(int)
    elif 'generated' in train_df.columns:
        train_df['is_generated'] = train_df['generated'].astype(int)
    
    if 'label' in valid_df.columns:
        valid_df['is_generated'] = (valid_df['label'] == 'ai').astype(int)
    elif 'generated' in valid_df.columns:
        valid_df['is_generated'] = valid_df['generated'].astype(int)
    
    print(f"  Train: {len(train_df)} samples")
    print(f"  Valid: {len(valid_df)} samples")
    
    # Show class distribution
    print(f"  Train class distribution: {train_df['is_generated'].value_counts().to_dict()}")
    print(f"  Valid class distribution: {valid_df['is_generated'].value_counts().to_dict()}")
...
def compute_detection_predictions(df, model_config):
    """Compute predictions using detection model."""
    try:
...
def main():
    print("="*70)
    print("ENSEMBLE EVALUATION ON FINAL DATASET")
    print("="*70)
    ...
    y_valid = valid_df['is_generated'].values
    
    results = {}
    all_preds = {'train': {}, 'valid': {}, 'test': {}}
    ...
    for name, config in DETECT_PREDICTIONS.items():
        print(f"\n{name}:")
        # Use valid_df as test set
        test_preds = load_detection_predictions(valid_df, name, config)
        
        if test_preds is not None:
            all_preds['test'][name] = test_preds
            metrics = calc_metrics(y_valid, test_preds, f"  {name} (valid)")
    ...
    emb_valid = get_embedding_knn_predictions(train_df, valid_df, k=5)
    
    if emb_train is not None:
        all_preds['train']['embedding_knn'] = emb_train
        all_preds['valid']['embedding_knn'] = emb_valid
        all_preds['test']['embedding_knn'] = emb_valid  # Use valid as test
        metrics = calc_metrics(y_valid, emb_valid, "  Embedding KNN (valid)")
    ...
    try:
        ...
        rank_valid = get_ranking_predictions_fast(train_df, valid_df)
        
        if rank_train is not None:
            all_preds['train']['ranking'] = rank_train
            all_preds['valid']['ranking'] = rank_valid
            all_preds['test']['ranking'] = rank_valid  # Use valid as test
            metrics = calc_metrics(y_valid, rank_valid, "  Ranking Model (valid)")
    ...
    for name in list(all_preds['test'].keys()):
        if name not in all_preds['train'] and 'embedding_knn' in all_preds['train']:
            ...
            scaled_valid = (emb_valid - emb_test_mean) / (emb_test_std + 1e-8) * test_std + test_mean
            
            all_preds['train'][name] = np.clip(scaled_train, 0, 1)
            all_preds['valid'][name] = np.clip(scaled_valid, 0, 1)
    ...
    with open(os.path.join(RESULTS_DIR, "final_dataset_cv_report.json"), 'w') as f:
    ...
    val_scores = {}
    for name in all_preds['valid'].keys():
        val_auc = roc_auc_score(y_valid, all_preds['valid'][name])
    ...
    ensemble_valid = weighted_average(
        {k: v for k, v in all_preds['valid'].items() if k in normalized_weights},
        {k: v for k, v in normalized_weights.items() if k in all_preds['valid']}
    )
    ...
    print("\n  Weighted Ensemble (valid):")
    test_metrics = calc_metrics(y_valid, ensemble_test, "  ")
    results['weighted_ensemble_valid'] = test_metrics
    ...
    if len(available_models) >= 2:
        ...
        valid_features = np.array([all_preds['valid'][k] for k in available_models]).T
        ...
        print("\n  Meta-Learner (valid):")
        test_metrics = calc_metrics(y_valid, meta_test_probs, "  ")
        results['meta_learner_valid'] = test_metrics
        
        # Save meta-learner
        with open(os.path.join(RESULTS_DIR, "meta_learner_final_dataset.pkl"), 'wb') as f:
            pickle.dump(meta_model, f)
    else:
        print("  Skipping meta-learner (need at least 2 models)")
        meta_test_probs = ensemble_test
        results['meta_learner_valid'] = results.get('weighted_ensemble_valid', {})
    ...
    with open(os.path.join(RESULTS_DIR, "final_dataset_ensemble_results.json"), 'w') as f:
        json.dump(results, f, indent=2)
    
    submission_df = pd.DataFrame({
        'id': range(len(ensemble_test)),
        'is_generated': meta_test_probs if 'meta_test_probs' in dir() else ensemble_test,
    })
    submission_df.to_csv(os.path.join(RESULTS_DIR, "final_dataset_ensemble_submission.csv"), index=False)
    
    summary_df = pd.DataFrame(results).T
    summary_df.to_csv(os.path.join(RESULTS_DIR, "final_dataset_ensemble_summary.csv"))
