...
4. [Ансамбль из 3 моделей](#ансамбль-из-3-моделей)
5. [Датасеты](#датасеты)
...
### 🎯 Пороговые значения (Thresholds) для моделей
...
#### 🔧 Рекомендации по порогам для каждой модели

**1. Qwen2.5-7B (Detection) — ✅ Отличное качество**
- **Порог:** `0.5`
- **Вероятности:** AI → ~0.95-1.0, Human → ~0.0-0.05
- **Диапазон рабочего порога:** [0.1, 0.9] — идеальное разделение
- **Метрики при 0.5:** Accuracy=100%, F1=100%, Precision=100%, Recall=100%

**2. T-lite-7B (Detection) — ⚠️ Требует осторожности**
- **Порог:** `0.5`
- **Вероятности:** AI → [0.7, 0.95], Human → [0.1, 0.45] (TN) или [0.55, 0.8] (FP)
- **Проблема:** 29% False Positive на human текстах
- **Метрики при 0.5:** Accuracy=71%, F1=77%, Precision=63%, Recall=100%
- **Рекомендация:** Использовать только в ансамбле, не отдельно!

**3. USER-bge-m3 (Embedding) — ✅ Отличное качество**
- **Порог:** `0.5`
- **Вероятности:** AI → ~0.95-1.0, Human → ~0.0-0.05
- **Метрики при 0.5:** Accuracy=100%, MRR=100%

**4. Ансамбль (3 модели) — ✅ Идеальное качество**
- **Порог:** `0.5`
- **Формула:** `ensemble_prob = 0.336*Qwen + 0.336*Embed + 0.328*T-lite`
- **Метрики при 0.5:** Accuracy=100%, F1=100%, Precision=100%, Recall=100%

---

#### 💻 Пример использования порогов в коде

```python
import numpy as np

# Qwen2.5-7B (идеальное разделение)
qwen_prob = 0.95  # пример вероятности для AI текста
qwen_pred = 1 if qwen_prob > 0.5 else 0  # AI

# T-lite-7B (осторожно: 29% FP на human!)
t_lite_prob = 0.75  # пример вероятности
t_lite_pred = 1 if t_lite_prob > 0.5 else 0  # AI (но может быть FP)

# USER-bge-m3 (идеальное разделение)
embed_prob = 0.92  # пример вероятности
embed_pred = 1 if embed_prob > 0.5 else 0  # AI

# Ансамбль (рекомендуется!)
ensemble_prob = 0.336 * qwen_prob + 0.336 * embed_prob + 0.328 * t_lite_prob
ensemble_pred = 1 if ensemble_prob > 0.5 else 0  # AI
```

---

#### 📈 Альтернативные пороги для T-lite-7B (если используете отдельно)

| Сценарий | Порог | Precision | Recall | Применение |
|----------|-------|-----------|--------|------------|
| **Макс. Recall** | 0.3 | ~0.50 | 1.0 | Критично не пропускать AI |
| **Баланс** | 0.5 | ~0.63 | 1.0 | Стандартное использование ⚠️ |
| **Макс. Precision** | 0.7 | ~0.85 | ~0.85 | Критично избегать FP |
| **Строгий** | 0.8 | ~0.95 | ~0.70 | Только уверенные предсказания |

> ⚠️ **Важно:** Пороги указаны для валидационного датасета `final_prepared/final_valid.csv` (491 сэмпл). На реальных данных пороги могут требовать калибровки. Рекомендуется использовать ансамбль из 3 моделей вместо отдельных моделей.
...
**Документация:** `CORRECT_SPLIT.md`, `ARCHITECTURE_V2.md`, `V2_README.md`  
**Отчеты:** `results/DETECTION_METRICS_REPORT.md`, `MODEL_ZOO.md`
